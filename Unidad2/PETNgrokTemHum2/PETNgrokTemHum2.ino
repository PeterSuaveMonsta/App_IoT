...
const char *ssid = "CGA2121_WnpBnUw";
const char *password = "fSjVwAqRgE7D8aF9Hz";
...
const String serverUrl = "http://8b01-189-195-208-73.ngrok-free.app"; // Reemplaza con la IP del servidor Python
...
void loop() {
  ...
  if (!isnan(humedad) && !isnan(temperaturaCelsius)) {
    ...
    sendTemperatureData(temperaturaCelsius);
    sendHumedadData(humedad);
  }
...
void sendTemperatureData(float temperatura) {
  HTTPClient http;
...
// Función para enviar los datos de humedad al servidor
void sendHumedadData(float humedad) {
  HTTPClient http;
  String url = serverUrl + "/humedad";  // Reemplaza con la ruta correcta en tu servidor

  // Construye el JSON para la solicitud POST
  String postData = "{\"humedad\":" + String(humedad) + "}";
